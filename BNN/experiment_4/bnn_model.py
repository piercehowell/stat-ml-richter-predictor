...
class RichterPredictorBNN(VIModule):
	...
	def __init__(self,
		...
		self.embedding1 = nn.Embedding(1428, 2) # geo_level_2_id
		self.embedding2 = nn.Embedding(12568, 2) # geo_level_3_id
		self.embedding3 = nn.Embedding(4, 2) #position
		self.embedding4 = nn.Embedding(5, 2) # foundation type
		self.embedding5 = nn.Embedding(3, 2) #roof_type
		self.embedding6 = nn.Embedding(5, 2) # ground_floor_type
		self.embedding7 = nn.Embedding(4, 2) # other_floor_type
		self.embedding8 = nn.Embedding(10, 2) # plan_configuration
		self.embedding9 = nn.Embedding(4, 2) # legal ownership
		self.embedding10 = nn.Embedding(3, 2) # land_surface_condition
		self.in_features = (self.in_features - 11) + 22
	...
	def forward(self, x, stochastic=True):
		...
		e9 = self.embedding9(x[:, 9].to(torch.int32))
		e10 = self.embedding10(x[:, 10].to(torch.int32))
		
		x = torch.cat((e0, e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, x[:, 11:]), dim=1)
